...
//    if ([type isEqualToString:@"0"])
//    {
        uri = @"/suser?action=Register";
//    }else{
//        uri = @"/suser?action=BindNewAccount";
//    }
...
//            NSLog(@"response ===== %@", responseObject);
            ...
            [CommonUtil saveObjectToUD:_phoneNum key:@"loginusername"];
...
- (BOOL)textFieldShouldEndEditing:(UITextField *)textField
{
    if (textField == self.trueNameTextField) {
        if (self.trueNameTextField.text.length == 0) {
            self.trueNameLine.backgroundColor = RGB(211, 211, 211);
        ...
        if (self.passwordTextField.text.length == 0) {
            self.passwordLine.backgroundColor = RGB(211, 211, 211);
