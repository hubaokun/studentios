...
- (void)settingView {
    ...
    if ([_isSkip isEqualToString:@"1"])
    {
        ...
        [self.backBtn setTitleColor:RGB(80, 203, 151) forState:UIControlStateNormal];
...
- (void)postPerfectStudentInfo {
    ...
    UIImage *cardPic1 = self.idCardFrontImageView.image;
    UIImage *cardPic2 = self.idCardBackImageView.image;
    UIImage *cardPic3 = self.licenseFrontImageView.image;
    UIImage *cardPic4 = self.licenseBackImageView.image;
    ...
    [paramDic setObject:_id_cardnum forKey:@"idnum"];
    [paramDic setObject:_student_cardnum forKey:@"studentcardnum"];
    [paramDic setObject:_student_card_creat forKey:@"scardyear"];
    
    NSString *uri = @"/suser?action=PerfectStudentInfo";
    ...
    [manager POST:[RequestHelper getFullUrl:uri] parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        // 添加图片数据
        if (cardPic1 != nil) {
            [formData appendPartWithFileData:UIImageJPEGRepresentation(cardPic1, 0.75) name:@"cardpic1" fileName:@"image.jpg" mimeType:@"image/jpeg"];
        }
        if (cardPic2 != nil) {
            [formData appendPartWithFileData:UIImageJPEGRepresentation(cardPic2, 0.75) name:@"cardpic2" fileName:@"image.jpg" mimeType:@"image/jpeg"];
        }
        if (cardPic3 != nil) {
            [formData appendPartWithFileData:UIImageJPEGRepresentation(cardPic3, 0.75) name:@"cardpic3" fileName:@"image.jpg" mimeType:@"image/jpeg"];
        }
        if (cardPic4 != nil) {
            [formData appendPartWithFileData:UIImageJPEGRepresentation(cardPic4, 0.75) name:@"cardpic4" fileName:@"image.jpg" mimeType:@"image/jpeg"];
        }
        ...
        if (code == 1) {
            ...
            // 数据本地化
            _id_cardpicfurl = responseObject[@"cardpic1url"];
            _id_cardpicburl = responseObject[@"cardpic2url"];
            _student_cardpicfurl = responseObject[@"cardpic3url"];
            _student_cardpicburl = responseObject[@"cardpic4url"];
            [self locateData];
